...
namespace ReportesRegulatorios.Modelos
{
    internal class EncaDv17
    {
        ...
        public DataTable ObtenerEncabezado(int anioMes)
        {
            ...
            string consulta = "select AnioMes, Estado, Usuario_genera, Fecha_genera, Usuario_upd, Fecha_upd, Usuario_Cierre, Fecha_Cierre, Doc_cierre from DL_CUMPLIMIENTO.rrdv17_detalle_enca where AnioMes=" + anioMes;
            try
            {
        ...
        )
        {
            try
            {
                ...
                {
                    string query = @"
                UPDATE EDW.DL_CUMPLIMIENTO.rrdv17_detalle_enca
                SET 
                    Estado = @Estado,
