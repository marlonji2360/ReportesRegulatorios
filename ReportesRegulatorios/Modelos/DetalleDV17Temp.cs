...
namespace ReportesRegulatorios.Modelos
{
    internal class DetalleDV17Temp
    {
        public DataTable ObtenerDetalleTmpCsv(int anioMes)
        {
            ...
            string consulta = "SELECT * FROM DL_CUMPLIMIENTO.rrdv17_detalle_tmp WHERE AnioMes = @AnioMes";
...
WHERE ISNULL(RRJJ.JUSTIFICACION_NEW,' ') = ISNULL(RRJJ.JUSTIFICACION_ORIGEN,' ')
   OR  ISNULL(RRJJ.JUSTIFICACION_NEW,' ') = ' '";
        ...
        public bool InsertarDetalleDv17TmpBulk(DataTable dataTable)
        {
            try
            {
        ...
        public bool InsertarDetalleDv17Tmp(DataTable dataTable)
        {
            try
            {
                Conexion conexion = new Conexion();
