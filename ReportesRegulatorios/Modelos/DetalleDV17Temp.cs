...
namespace ReportesRegulatorios.Modelos
{
    ...
    {
        public DataTable ObtenerDetalleTmpCsv(int anioMes ,string tipoConexion)
        {
            ...
            string consulta = @"SELECT  AnioMes, 
                                        ...
                                        hora_trx, 
                                        cajero
                                    FROM EDW.DL_CUMPLIMIENTO.rrdv17_detalle_tmp
            ...
            string consulta = @"SELECT
                                ...
                                FROM (	SELECT  SUM(XX.CASOS_ORIGEN) CASOS_ORIGEN,
		                                ...
		                                SELECT COUNT(*) CASOS_ORIGEN, 0 CASOS_REVISAR
		                                 ...
		                                 FROM EDW.DL_CUMPLIMIENTO.RRDV17_DETALLE_TMP DRDDT
        ...
        {
            string consulta = @"DELETE FROM EDW.DL_CUMPLIMIENTO.rrdv17_detalle_tmp WHERE AnioMes = @anioMes";
...
WHERE ISNULL(RRJJ.JUSTIFICACION_NEW,' ') = ISNULL(RRJJ.JUSTIFICACION_ORIGEN,' ')
   OR  ISNULL(RRJJ.JUSTIFICACION_NEW,' ') = ' '";
        ...
        public bool InsertarDetalleDv17TmpBulk(DataTable dataTable, string tipoConexion)
        {
            try
            {
        ...
        public bool InsertarDetalleDv17Tmp(DataTable dataTable, string tipoConexion)
        {
            try
            {
                if(tipoConexion == "Principal")
                {
                    ...
                    using (SqlConnection conn = conexion.AbrirConexion())
                    {
                        foreach (DataRow row in dataTable.Rows)
                        {
                            using (SqlCommand cmd = new SqlCommand("DL_CUMPLIMIENTO.PRC_Insertar_DV17_tmp", conn))
                            {
